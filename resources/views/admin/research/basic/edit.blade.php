...
<div class="card">
    <div class="card-header">
        {{ trans('global.update') }} {{ trans('cruds.research.title') }} {{ trans('cruds.research.basic') }}
        <hr>
    </div>

    <div class="card-body">

        <form id="ResearchBasicEditForm" enctype="multipart/form-data"> 

            <input type="hidden" value="{{ $research->id }}" id="researchID">

            <div class="form-group" id="research_details_image_preview_block">
                ...
                <img 
                    ...
                    src="{{ asset('images/research/'.$research->research_homepage_image ) }}">
            </div>
            ...
            <div class="form-group" id="research_main_image_preview_block">
                <label for="research_image_preview">{{ trans('cruds.research.fields.preview_details_image') }}</label>
                <img 
                    id="research_main_image_preview" 
                    class="d-block border-0 mt-1"  
                    style="height:150px; width:150px;" 
                    src="{{ asset('images/research/'.$research->research_details_main_image ) }}">
            </div>
            ...
            <div class="form-group {{ $errors->has('research_title') ? 'has-error' : '' }}">
                ...
                <input type="text" id="research_title" name="research_title" class="form-control" value="{{ old('research_title', isset($research) ? $research->research_title : '') }}" required>
            ...
            <div class="form-group {{ $errors->has('research_title_arabic') ? 'has-error' : '' }}">
                ...
                <input 
                    ...
                    dir="rtl" 
                    value="{{ old('research_title_arabic', isset($research) ? $research->research_title_arabic : '') }}" 
                    required
                >
            ...
            <div class="form-group {{ $errors->has('research_description') ? 'has-error' : '' }}">
                <label for="research_description">{{ trans('cruds.research.fields.main_description') }}*</label>
                    <div class="editor-full">
                        <input name="research_description" id="research_description_input" type="hidden" value="{{ old('research_description', isset($research) ? $research->research_description : '') }}">
                        <div id="research_description" class="ql-scroll-y" style="height: 300px;">
                            {{ old('research_description', isset($research) ? $research->research_description : '') }}
                        </div>
                    </div>
            ...
            <div class="form-group {{ $errors->has('research_description_arabic') ? 'has-error' : '' }}">
                ...
                <div class="editor-full">
                    <input name="research_description_arabic" id="research_description_arabic_input" type="hidden" value="{{ old('research_description_arabic', isset($research) ? $research->research_description_arabic : '') }}">
                    <div id="research_description_arabic" dir="ltr" class="ql-scroll-y text-right" style="height: 300px;">
                        {{ old('research_description_arabic', isset($research) ? $research->research_description_arabic : '') }}
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    researchDes.root.innerHTML = $("#research_description_input").val();
    researchDesArab.root.innerHTML = $("#research_description_arabic_input").val();
    ...
    $("#ResearchBasicEditForm").on('submit', function(e) {
        e.preventDefault();
        ...
        let id = $("#researchID").val();
        formData.append('id', id);

        $.ajax({
            url: "{{ route('admin.update-research')}}",
            type: 'POST',
            data: formData,
            dataType:'JSON',
            contentType: false,
            ...
            success: function(response) {
                toastr.success("Research Basic Information updated successfully");
                setTimeout(function(){
